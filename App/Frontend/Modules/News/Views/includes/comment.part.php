<fieldset data-id="<?= $comment['id'] ?>">
	<legend>
		...
		<?php if ( $user->isAuthenticated() && $user->isAdmin() ): ?> -
			<a href="<?= \OCFram\RouterFactory::getRouter( 'Backend' )->getUrl( 'News', 'updateComment', false, [ 'id' => $comment[ 'id' ] ] ) ?>"
			>Modérer</a> |
			<a href="<?= \OCFram\RouterFactory::getRouter( 'Backend' )->getUrl( 'News', 'deleteComment', false, [ 'id' => $comment[ 'id' ] ] ) ?>"
			   class="js-delete-comment"
			   data-action="<?= \OCFram\RouterFactory::getRouter( 'Backend' )->getUrl( 'News', 'deleteCommentJson', 'json', [ 'id' => $comment[ 'id' ] ] ) ?>"
			>Supprimer</a>
		<?php endif; ?>
